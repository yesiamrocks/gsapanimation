(function (factory) {
    ...
    function animateFadeIn(el) {
      ...
      if (from && to) {
        ...
        if (typeof options.from === 'undefined' && typeof options.fromOpacity === 'undefined' && typeof options.opacity === 'undefined') {
          gsap.set(el, {
            opacity: 0
          });
     ...
     * @param {Object} [options={}] - Optional GSAP overrides (from, to, etc.)
     */
    function animateFadeInRight(el) {
      let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
      const from = options.from || null;
      const to = options.to || null;
      const defaultFrom = {
     ...
     */
    function wrapLettersIfNeeded(el) {
      let spanClass = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'ca__gsap-letter';
      ...
      const chars = [...text];
      el.innerHTML = chars.map(char => char === ' ' ? ' ' : `<span class="${spanClass}">${char}</span>`).join('');
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      animatedElements.forEach(el => {
        ...
        if (typeof animateFn !== 'function') {
          console.warn(`[cssanimation.io] ⚠️ Unknown animation: "${animationName}" not found in animationMap.`);
        ...
        if (hasOptions) {
          try {
            ...
            console.group(`[cssanimation.io] ❌ Invalid JSON in ca-gsap-options`);
        ...
        const target = isLetter ? wrapLettersIfNeeded(el, 'ca__gsap-letter') : el;
        if (!isLetter) {
...
//# sourceMappingURL=gsapanimation.umd.js.map
